...
<Aside type="note" title="권한 평가 순서">
  Claude Code SDK의 권한 평가는 훅, deny 규칙, permission mode, allow 규칙, 런타임 승인 콜백 순서로 진행됩니다. `bypassPermissions`도 deny 규칙과 훅을 모두 무시하는 것은 아니며, `dontAsk`는 승인 콜백을 호출하지 않고 미승인 도구를 거절합니다.
</Aside>
