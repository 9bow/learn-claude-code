...
Remote Control은 환경 변수로 동작 모드를 바꾸는 기능이 아닙니다. 다른 기기에서 로컬 세션을 이어서 제어하려면 공식 문서의 `claude --remote-control`, `claude remote-control`, 또는 대화형 `/remote-control` 흐름을 사용합니다. 이 기능은 연구 미리보기이며, 로컬 파일·도구·MCP 권한이 그대로 적용됨을 이해하고 시작해야 합니다.
